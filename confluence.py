...
MULTIPART_HEADERS = {"X-Atlassian-Token": "nocheck"}  # Only need this for form uploads
...
class Confluence:
    ...
    def _request(
        self, method="GET", path="", params=None, files=None, data=None, headers=None
    ):
    ...
    def post(self, path=None, params=None, data=None, files=None):
        return self._request(
            method="POST", path=path, params=params, data=data, files=files
        )
    ...
    def create_labels(
        self, page_id: str, id_label: str, tags: Optional[List[str]] = None
    ):
    ...
    def create(
        ...
        if "_links" in response:
            ...
            log.info(
                'Article "{title}" (id {page_id}) created successfully at {url}'.format(
            ...
            log.info(
                'Failed to update article "{title}" (id {page_id}).'.format(
    ...
    def update(
        ...
        if "_links" in response:
            ...
            log.info(
                'Article "{title}" (id {page_id}) updated successfully at {url}'.format(
